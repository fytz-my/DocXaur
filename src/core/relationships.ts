...
export function ensureImageRelationships(
  ...
  const base = (relsXml && relsXml.trim().length) ? relsXml : `${xmlHeader}${openTag}${closeTag}`;

  const existingIds = new Set(
    (base.match(/Id="(rId\d+)"/g) ?? []).map(s => s.replace(/.*Id="(rId\d+)".*/, "$1")),
  );
  const additions = images
    .filter(img => !existingIds.has(img.rid))
    .map(img => `<Relationship Id="${img.rid}" Type="${IMAGE_REL}" Target="${img.target}"/>`)
    .join("");
