    ...
    import { ptToHalfPoints, parseNumberTwips, base64ToUint8Array, fetchImageAsBase64 } from "./utils.ts";
    import { ensureImageRelationships } from "./relationships.ts";
    import { Section } from "../blocks/section.ts";

    export interface DocumentOptions {
      title?: string; creator?: string; description?: string;
      subject?: string; keywords?: string; fontName?: string; fontSize?: number;
    }
    export interface SectionOptions { pageSize?: PageSize; margins?: Margins; }
    export interface PageSize { width: string; height: string; orientation?: "portrait" | "landscape"; }
    export interface Margins { top: string; right: string; bottom: string; left: string; }

    export interface ImageOptions { width?: string; height?: string; align?: "left" | "center" | "right" | "justify"; }
    export interface ParagraphOptions {
      align?: "left" | "center" | "right" | "justify";
      bold?: boolean; italic?: boolean; underline?: boolean;
      fontSize?: number; fontColor?: string; fontName?: string;
      spacing?: { before?: number; after?: number; line?: number; };
      breakBefore?: number; breakAfter?: number;
    }
    export interface TableColumn {
      width: string;
      hAlign?: "left" | "center" | "right" | "justify";
      vAlign?: "top" | "center" | "bottom";
      fontName?: string; fontSize?: number; fontColor?: string; cellColor?: string;
      bold?: boolean; italic?: boolean; underline?: boolean;
    ...
    export class DocXaur {
      ...
      getImageRelId(imageId: number): string { return `rId${imageId + 3}`; }

      private async createDocxZip(): Promise<Blob> {
        const blobWriter = new BlobWriter();
        const zipWriter  = new ZipWriter(blobWriter);

        await zipWriter.add("[Content_Types].xml", new TextReader(this.generateContentTypes()));
        ...
        await zipWriter.add("word/_rels/document.xml.rels", new TextReader(fixedDocRels));
        await zipWriter.add("word/document.xml", new TextReader(docXml));
        await zipWriter.add("word/styles.xml",    new TextReader(this.generateStyles()));
        await zipWriter.add("word/fontTable.xml", new TextReader(this.generateFontTable()));
        await zipWriter.add("word/settings.xml",  new TextReader(this.generateSettings()));

        for (const [, imgData] of this.images) {
          const filename = `word/media/image${imgData.id}.${imgData.extension}`;
          await zipWriter.add(filename, new BlobReader(new Blob([base64ToUint8Array(imgData.data)])));
        }
      ...
      async toBlob(): Promise<Blob> { return await this.createDocxZip(); }
...
`;
        const sorted = Array.from(this.images.entries()).sort((a, b) => a[1].id - b[1].id);
        for (const [, d] of sorted) {
          const relIdNum = d.id + 3;
          xml += `  <Relationship Id="rId${relIdNum}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/image${d.id}.${d.extension}"/>
`;
        }
